...
module.exports = class Parameters {
  ...
  _validate_ (ctx, type = 'query', originData = {}) {
    ...
    const defaultKey = 'koa-oai-router:default:required:just4validatequery';
    if (type === 'query' && jsonSchema.required && jsonSchema.required.length === 0) {
      jsonSchema.required.push(defaultKey);
      jsonSchema.properties[defaultKey] = {"type": "string"};
      originData[defaultKey] = defaultKey;
    }

    const {valid, message, path, error, data} = validate(originData, jsonSchema, this.formater, this.validator);
    delete originData[defaultKey];

    debug(`validate [${type}] [${valid}] [${this.validator}]`, jsonSchema, data, error);
    if (valid) return;
