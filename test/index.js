...
test('ctx.module required in multi module', t => {
  const fn = invokeParser(undefined, {
    modules: [1]
  });
...
test('empty controller', t => {
  ...
  const fn1 = invokeParser(undefined, {
    modules: []
  });
   ...
   */
  const fn2 = invokeParser(undefined, {
    modules: ['baz']
  }); 
  ...
  const fn3 = invokeParser(undefined, {
    modules: ['baz'],
    controllers: {} 
  });
...
test('__before', async t => {
  let recieved = 0;
  let expected = 0;
  const getFn = __before => invokeParser(undefined, {
    modules: [],
    controllers: {
      foo: class {
        constructor () {
